...
export default function RestaurantDetails() {
  ...
  useEffect(() => {
    async function load() {
      try {
        // Tenta endpoint de detalhe; se não existir, cai no fallback com a lista
        ...
        // Carrega foods para montar o "cardápio" do restaurante
  ...
  // Seleciona itens do cardápio deste restaurante
  const menuFoods = useMemo(() => {
    if (!restaurant) return [];
    const rid = restaurant.id;
    const rname = (restaurant.name || '').toLowerCase();
    return foods.filter(f => {
      // tenta por restaurantId; fallback por restaurantName (se existir no dataset)
    ...
    <div className="container py-4">
      ...
      <div className="row g-4 align-items-start">
        ...
        <div className="col-12 col-md-7">
          ...
          {/* Informações extras (se existirem no dataset) */}
